...
PlayerBotMgr::PlayerBotMgr() :
    ...
    m_LFGSearchTick(0),
    m_ArenaSearchTick(0),
    m_LastPlayerBotCreateFinished(false)
{
...
PlayerBotCreateInfo PlayerBotMgr::BuildCreatePlayerData(bool group, uint8 prof)
{
    PlayerBotCreateInfo info;
    info.race = RandomRace(group, prof);
    info.gender = irand(0, 1);
    info.skinColor = RandomSkinColor(info.race, info.gender, prof);
    info.faceID = RandomFace(info.race, info.gender, info.skinColor, prof);
    info.hairID = RandomHair(info.race, info.gender, prof);
    info.hairColor = RandomHairColor(info.race, info.gender, info.hairID, prof);
    info.facialHair = RandomFacialHair(info.race, info.gender, info.hairColor, prof);
    info.playerClass = prof;
...
void PlayerBotMgr::CreateOncePlayerBot()
{
    while (true)
    {
        ...
        PlayerBotCreateInfo createInfo = pInfo->needCreateBots.front();
        ...
        WorldPackets::Character::CreateChar createChar(WorldPacket(CMSG_CREATE_CHARACTER));
        createChar.CreateInfo = std::make_shared<WorldPackets::Character::CharacterCreateInfo>();
        createChar.CreateInfo->Name = name;
        createChar.CreateInfo->Race = createInfo.race;
        createChar.CreateInfo->Class = createInfo.playerClass;
        createChar.CreateInfo->Sex = createInfo.gender;
        createChar.CreateInfo->Skin = createInfo.skinColor;
        createChar.CreateInfo->Face = createInfo.faceID;
        createChar.CreateInfo->HairStyle = createInfo.hairID;
        createChar.CreateInfo->HairColor = createInfo.hairColor;
        createChar.CreateInfo->FacialHairStyle = createInfo.facialHair;
        createChar.CreateInfo->OutfitId = 0;

        pSession->HandleCharCreateOpcode(createChar);

        // In the synchronous path (SkipCinematics = 0) OnPlayerBotCreate is
        // called inside HandleCharCreateOpcode, so keep creating the rest.
        // In the asynchronous path the creation finishes later and
        // PlayerBotMgr::Update resumes processing.
        if (!m_LastPlayerBotCreateFinished)
            break;
    }
}

void PlayerBotMgr::ResumePlayerBotCreate()
{
    for (std::map<uint32, PlayerBotBaseInfo*>::iterator it = m_idPlayerBotBase.begin();
        it != m_idPlayerBotBase.end();
        it++)
...
{
    m_LastPlayerBotCreateFinished = true;
